...
def dashboard():
    ...
    medications = query_db('''
        SELECT name, dosage, start_date, end_date FROM medications
        WHERE user_id = ?
    ...
    reminders = query_db('''
        ...
        FROM reminders
        WHERE user_id = ? AND datetime(reminder_time) >= datetime('now') AND (is_completed = 0 OR is_completed IS NULL)
...
def reminders_page():
    ...
    now = datetime.now().strftime("%Y-%m-%d %H:%M")  # Şu anki zaman
    now_plus_hour = (datetime.now() + timedelta(hours=1)).strftime("%Y-%m-%d %H:%M")  # Şu an +1 saat
    return render_template('reminders.html', reminders=reminders, now=now, now_plus_hour=now_plus_hour)
...
@app.route('/api/reminders_due', methods=['GET', 'POST'])
def api_reminders_due():
    ...
    if request.method == 'POST':
        # Bildirilen hatırlatıcıyı tamamlama
        reminder_id = request.json.get('reminder_id')
        if reminder_id:
            execute_db('UPDATE reminders SET is_completed = 1 WHERE id = ? AND repeat_interval = "yok"', [reminder_id])
        return jsonify(success=True)

    reminders = query_db(
        '''SELECT id, title, repeat_interval FROM reminders 
           WHERE user_id = ? 
             AND datetime(reminder_time) <= datetime(?) 
             AND (is_completed = 0 OR is_completed IS NULL)''',
        [user_id, now]
    )
    reminders_list = [dict(id=r['id'], title=r['title'], repeat_interval=r['repeat_interval']) for r in reminders]
